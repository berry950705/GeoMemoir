...
(function(){
        ...
        var show_story_img = $('#show_story_img');
        var story_img = $('#story_img');
        var photoDesc = $('#photo_desc');
        var summernote_en = $('#summernote_en');
        var summernote_tw = $('#summernote_tw');
        var summernote_cn = $('#summernote_cn');
        var btn_en = $('#lang_en');
        var btn_tw = $('#lang_tw');
        var btn_cn = $('#lang_cn');
        ...
        $.ajax({
                type:"get",
                url: baseUrl+ cur_lang + '/v1/stories/'+username+'/get_story?story_num='+story_id,
                dataType: 'json',
                ...
                success: function(response){
                        if (response.code==200){
                                ...
                                var country = story['country']
                                var photo = story['photo']
                                var photo_desc = story['photo_desc']
                                var content = story['content']

                                story_title.val(title)
                                check_area(area);
                                check_category(category);
                                add_countries(country);
                                photoDesc.text(photo_desc)
                                show_story_img.attr('src', imgUrl+ photo)
                                summernote_en.summernote('pasteHTML', content)
                                summernote_tw.summernote('pasteHTML', content)
                                summernote_cn.summernote('pasteHTML', content)
                        }else{
                                alert(response.msg)
                        }
                }       
        ...
        //update edited story content 
        $('#update_btn').click(function(){

                if(story_title.val() &&get_checkArea()&&get_checkCateg()&&countries.val()&&photoDesc.val()&&get_editContent()){
        
                        formdata = new FormData()
                        if (story_img[0].files[0]){
                                formdata.append("photo", story_img[0].files[0])
                        }
                        formdata.append('title', story_title.val())
                        formdata.append('area', get_checkArea())
                        formdata.append('category', get_checkCateg())
                        formdata.append( 'country', countries.val())
                        formdata.append('photo_desc', photoDesc.val())
                        formdata.append('content', get_editContent())

                        $.ajax({
                                processData: false,
                                contentType: false,
                                type: 'post',
                                url: baseUrl+ cur_lang + '/v1/stories/' + username+'/update_story?story_num=' +  story_id,
                                data: formdata,
                                beforeSend: function(request){
                                        request.setRequestHeader('Authorization', token)
                                },               
                                success: function(response){
                                        if (response.code == 200){
                                                var message = editStory_messages[0][cur_lang]
                                                alert (message)
                                                setTimeout(function(){
                                                        window.location.href = '/'+cur_lang+'/'+username+'/story_list' 
                                                }, 1)
                                        }else{
                                                alert(response.msg)
                                        }
                        ...
                        var message = editStory_messages[1][cur_lang]
                        alert(message)
                }
        })
        ...
        function add_countries(find){
                var country_code = find.split(',')[0]
                if (cur_lang=='en'){
               ...
               $.each(countries_list, function(i, obj){
                        if(country_code.trim() == obj['iso_code']){
                                option = '<option selected>'+obj['iso_code']+', '+obj['country_name']+'</option>'
                                countries.append(option)
                        }else{
                                option = '<option>'+obj['iso_code']+', '+obj['country_name']+'</option>'
                                countries.append(option)
                        }
                })                
        }               
        
        function check_area(find){
                if (find == area1.attr('value')){
                        area1.prop('checked', true);
                }else if (find == area2.attr('value')){
                        area2.prop('checked', true);
                }else if(find == area3.attr('value')){
                        area3.prop('checked', true);
                }else{
                        area4.prop('checked', true);
                }       
        }

        function check_category(find){
                if (find == category1.attr('value')){
                        category1.prop('checked', true);
                }else if (find == category2.attr('value')){
        ...
        function get_checkArea(){
                if (area1.is(':checked') == true){
                        return area1.attr('value');
                }else if(area2.is(':checked') == true){
                        return area2.attr('value');
                }else if(area3.is(':checked') == true){
                        return area3.attr('value');
                }else if(area4.is(':checked') == true){
                        return area4.attr('value');
                }else{
                        return false;
                }
        }

        function get_checkCateg(){
                if (category1.is(':checked') == true){
                        return category1.attr('value');
                }else if(category2.is(':checked') == true){
                        return category2.attr('value');
                }else if(category3.is(':checked') == true){
                        return category3.attr('value');
                }else{
                        return false;
                }
        }

        function get_editContent(){
                if (btn_en.hasClass('chosen_lang') && summernote_en.summernote('code')!='<p><br></p>'){
                        return summernote_en.summernote('code')
                }else if (btn_tw.hasClass('chosen_lang') && summernote_tw.summernote('code')!='<p><br></p>'){
                        return summernote_tw.summernote('code')
                }else if (btn_cn.hasClass('chosen_lang') && summernote_cn.summernote('code')!='<p><br></p>'){
                        return summernote_cn.summernote('code')
                }else{
                        return false;
                }
        }
        ...
        const editStory_messages = [
                {
                'en': 'Story has been successfully  updated.',
                'zh_Hant_TW': '您已成功更新故事',
                'zh_Hans_CN': '您已成功更新故事' 
                },
                {
                'en': 'Please complete the story before uploading.',
                'zh_Hant_TW': '請您完成故事內容再行上傳',
                'zh_Hans_CN': '请您完成故事内容再行上传'        
                },
        ]


        // Test get data
        // btn_en.mouseover(function(){
        //         console.log(story_title.val())
        //         content =  get_editContent()
        //         console.log(content)
        //         summernote_ch.summernote('code', content)
        //        console.log('checked Area:', get_checkArea())
        //        console.log('checked Category:', get_checkCateg())
        //        console.log('selected Country', countries.val())
        //        console.log(photoDesc.val())
        // })
        // test add radio checked_attr
        // btn_ch.mouseover(function(){
        //         console.log('Works!!')
        //         check_area('The Americas')
        //         check_category('food')
